...
class CatData(data.Data):
    ...
    def __init__(self, filepath=None, headers=None, data=None, header2col=None, cats2levels=None):
        ...
        self.dataCopy = super().get_all_data()
    ...
    def get_cat_levels_str(self, header):
        ...
        '''
        cats2levels = self.get_cat_level_mappings()
        return np.array(cats2levels[header])
    ...
    def get_cat_levels_int(self, header):
        ...
        '''
        templist = []
        levelStrings = self.get_cat_levels_str(header)
        cats2levels = self.get_cat_level_mappings()

        for level in levelStrings:
            templist.append(cats2levels[header].index(level))
    ...
    def int2strlevels(self, header, int_levels):
        ...
        '''
        catLevelsStr = self.get_cat_levels_str(header)
        templist = []

        for int in int_levels:
            templist.append(catLevelsStr[int])
    ...
    def str2intlevels(self, header, str_levels):
        ...
        '''
        catLevelsInt = self.get_cat_levels_int(header)
        catLevelsStr = self.get_cat_levels_str(header)
        templist = []

        for str in str_levels:
            location = np.where(catLevelsStr == str)[0][0]
            templist.append(catLevelsInt[location])
    ...
    def filter(self, header, strlevel):
        ...
        '''
        intLevels = self.str2intlevels(header, [strlevel])[0]
        colIndex = self.header2col[header]

        if not isinstance(self.data, np.ndarray):
            self.data = np.array(self.data)
        
        filteredData = self.data[np.squeeze(self.data[:, colIndex] == intLevels)]
