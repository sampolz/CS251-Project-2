...
def sidebarplot(values, labels, title, show_counts=True, figsize=(6, 7), sort_by='na', ylabel = ''):
    ...
    plt.ylabel(ylabel)
    valsSorted, labelsSorted = sort(values,labels,sort_by)
    ax = plt.barh(labelsSorted,valsSorted)
    if show_counts:
        valsSorted = np.round(valsSorted, 2)
        plt.bar_label(ax, valsSorted)
    plt.title(title)
...
def sort(values, labels, sort_by='na'):
    ...
    if sort_by == 'value':
        sortOrder = np.argsort(unsorted1)
        for val in sortOrder:
            valuesSorted.append(unsorted1[val])
            labelsSorted.append(unsorted2[val])
    elif sort_by == 'label':
        sortOrder = np.argsort(unsorted2)
        for val in sortOrder:
            valuesSorted.append(unsorted1[val])
            labelsSorted.append(unsorted2[val])
    else:
        valuesSorted = unsorted1
        labelsSorted = unsorted2
    return valuesSorted,labelsSorted
